...
| R3 | «at least one Google Cloud infrastructure service (Cloud Run, Cloud SQL, Firestore, GKE, Pub/Sub)» | 🔲 | кандидат: Cloud Run + Firestore |
| R4 | «Must demonstrate the backend is running on Google Cloud» (Cloud Console, Cloud Run dashboard, Vertex AI logs, URL вида `.run`) | 🔲 | пруф в видео |
| R5 | «a next-generation, autonomous AI Agent leveraging Gemini 3.5 that operates beyond standard chat loops» | 🔲 | асинхронный батч-разбор корпуса |
...
| S6 | Hosted project URL | «encouraged but not strictly required for judging» | 🔲 |
| S7 | Пруф деплоя на Google Cloud | в видео и/или репо (= R4) | 🔲 |
