# 07 · Архитектура

Документ-артефакт подачи (требование S4). Диаграмма ниже рендерится прямо на GitHub;
экспорт в PNG для Devpost — `docs/img/architecture.png`, **на английском**:
> его смотрит жюри, а README и текст заявки тоже английские. Русская схема ниже —
> рабочая, английская собирается из `docs/architecture-en.mmd` командой
> `npx @mermaid-js/mermaid-cli -i docs/architecture-en.mmd -o docs/img/architecture.png -b white -w 2400`.
> При правке схемы править **обе**: расхождение диаграмм хуже отсутствия одной из них.

Правило этого файла: диаграмма отражает **то, что работает**, а не то, что задумано.
Каждый блок соответствует существующему модулю; пунктиром показано то, что ещё не
сделано, и это подписано явно.

---

## Общая схема

```mermaid
flowchart LR
    UI["<b>Веб-страница</b><br/>app/static/index.html<br/><i>отдаётся тем же сервисом,<br/>без сборки и CDN</i>"]
    UI --> DOI
    UI --> FILES

    DOI(["DOI"]) --> L0
    FILES(["Файлы от пользователя<br/>.pdf / .docx — путь B"]) --> L0

    L0["<b>L0</b><br/>pipeline.py<br/>оркестратор"]

    L0 --> EPMC
    L0 --> PDFP
    L0 --> SUP
    L0 --> UPW

    subgraph L1G["L1 · retrieval.py — добыча"]
        EPMC["Europe PMC REST<br/>метаданные + JATS"]
        PDFP["pdf_tables.py<br/>pdfplumber, текст + таблицы"]
        SUP["supplementaryFiles<br/>.docx приложений"]
        UPW["Unpaywall<br/>запасной канал"]
        LVL{{"assess_level<br/>что реально достали<br/>→ L1 / L2 / L3"}}
        EPMC --> LVL
        PDFP --> LVL
        SUP --> LVL
        UPW --> LVL
    end

    subgraph L2G["L2 · разбор таблиц"]
        JATS["jats_tables.py<br/>colspan / rowspan<br/>составные заголовки"]
        DOCX["docx_tables.py<br/>w:tbl / w:tr / w:tc"]
    end

    LVL --> JATS
    LVL --> DOCX

    JATS --> L3
    DOCX --> L3

    subgraph L3G["L3 · разбор моделью — три прохода параллельно"]
        L3["<b>critic.py</b><br/>Gemini 3.7 Flash · Vertex<br/>ROBINS-E, 7 доменов"]
        SUB["<b>subagents.py</b><br/>сопоставимость групп<br/>тест на противоречие"]
        SUB2["<b>subagents.py</b><br/>временнáя структура<br/>new-user, lag, латентность<br/>градиент по длительности"]
    end
    L3 --> L4
    SUB --> L4
    SUB2 --> L4
    L4["<b>L4 · verify_numbers.py</b><br/>каждое число —<br/>обратно в источник<br/>сверка меток групп<br/>→ GROUP_MISMATCH"]
    L4 --> L5
    L5["<b>L5 · stats_tool.py + recompute.py</b><br/>E-value, ARR, NNT, RR CI<br/><i>считает функция,<br/>заявленное моделью<br/>сравнивается с ней</i>"]
    L4 --> L5D
    L5D["<b>L5b · direction.py</b><br/>счёт направлений по доменам<br/>против общего вывода модели<br/>→ consistent / contradicts"]
    L5 --> OUT
    L5D --> OUT
    OUT["<b>Отчёт</b><br/>уровень<br/>+ находки по 7 доменам<br/>+ протокол сверки чисел<br/>+ пересчёт и свод направлений"]

    OUT --> SVC
    OUT --> JOB
    OUT --> STORE
    STORE["<b>store.py + brief.py</b><br/>постоянный id разбора<br/><code>/audits/{id}</code><br/>бриф на одну страницу"]

    subgraph L6["L6 · выполнение в облаке"]
        SVC["Cloud Run<br/><code>GET /</code> — страница<br/><code>POST /analyze</code> — API<br/>синхронно, ~50 с"]
        JOB["Cloud Run Job<br/>корпус, 3 воркера<br/>фоново"]
        GCS[("GCS<br/>объект на статью")]
        JOB --> GCS
        GCS -.->|"GET /runs/{id}"| SVC
    end

    SVC -.->|"отчёт на экран"| UI

    style UI fill:#e6f4ea,stroke:#34a853,stroke-width:2px
    style L3 fill:#e8f0fe,stroke:#4285f4,stroke-width:2px
    style SUB fill:#e8f0fe,stroke:#4285f4,stroke-width:2px
    style SUB2 fill:#e8f0fe,stroke:#4285f4,stroke-width:2px
    style L4 fill:#fce8e6,stroke:#ea4335,stroke-width:2px
    style L5 fill:#e6f4ea,stroke:#34a853,stroke-width:2px
    style L5D fill:#e6f4ea,stroke:#34a853,stroke-width:2px
    style STORE fill:#fef7e0,stroke:#fbbc04,stroke-width:2px
    style OUT fill:#fef7e0,stroke:#fbbc04,stroke-width:2px
```

---

## Что на схеме важно и почему именно так

### Уровень определяется добычей, а не назначается

`assess_level` в конце слоя 1 — не украшение, а несущая конструкция. Он смотрит, что
физически удалось достать, и присваивает уровень, который **ограничивает максимально
допустимую уверенность** всего отчёта:

| Уровень | Что достали | Потолок уверенности |
|---|---|---|
| L1 | full-text + таблицы приложения | `CONFIRMED` |
| L2 | полный текст без приложений | `PLAUSIBLE` / `UNVERIFIED` |
| L3 | только абстракт | `PLAUSIBLE` / `UNVERIFIED` |

Обоснование потолка — замеренное, а не выбранное, но не то, которым мы его обосновывали
сначала. Проверка (F-44) показала: на абстракте модель называет направление смещения
**верно**. Разница в другом — там она обосновывает его рассуждениями, подходящими любой
наблюдательной работе, а на L1 подставляет число из документа (7.8% против 5.9%,
Table A2). Проверять на нижних уровнях нечего, потому что цитировать нечего; верная
догадка доказательством не является. Поэтому `CONFIRMED` ниже L1 недостижим структурно.

### Верификатор стоит **после** модели, а не внутри неё

Красный блок L4 — главное архитектурное решение проекта.

Попросить модель проверить саму себя — значит спросить второе мнение у того же
процесса. Слой 4 берёт каждое число, которое модель написала в отчёте, находит его в
добытом источнике вместе с окружающим контекстом и **сравнивает метки групп**. Поэтому
он ловит не только выдуманное значение (`UNVERIFIED`), но и подменённое направление
(`GROUP_MISMATCH`) — когда число настоящее, а приписано не той группе.

Это ровно тот дефект, который проект ищет в чужих работах. Он был допущен и внутри
проекта — в нашем собственном эталоне (F-12), — и найден инструментом, а не глазами.

### Статистика вычисляется, а не генерируется

Зелёный блок L5 не обращается к модели вообще. E-value, ARR, NNT и доверительные
интервалы считает `stats_tool.py`, который самопроверяется при импорте. Ветки
`computed` и `arithmetic` исключены из сверки чисел, потому что проверять их надо не
поиском по тексту, а пересчётом.

**Так было написано раньше, чем стало правдой.** До 29.08 `stats_tool` в прямом пути
не вызывался ни разу — он был импортирован в `pipeline.py` и работал только внутри
графа ADK как инструмент агента, а на движке по умолчанию ARR и NNT писала модель
(F-55). Теперь пересчётом занят `recompute.py`: он собирает таблицу 2×2 из четырёх
чисел, которые модель обязана выписать в поле `computed.counts`, зовёт `stats_tool` и
кладёт рядом заявленное моделью с вердиктом `match` / `mismatch`. Сами четыре числа
идут в сверку наравне с остальными — они не результат арифметики, а выписка из
документа. Если таблица 2×2 не собирается, отчёт говорит об этом прямо, а не показывает
числа модели под заголовком «посчитано функцией».

### Направление смещения считается, а не утверждается

Блок L5b (`direction.py`) складывает направления семи доменов и сверяет счёт с общим
направлением, которое назвала модель. Это не взвешенный вердикт: домены не равновелики,
весов у нас нет и брать их неоткуда. Это проверка на противоречие — случай, когда пять
доменов из семи тянут в одну сторону, а итог назван другой, теперь виден в отчёте, а не
остаётся внутри модели.

### Два режима исполнения, одно ядро

Cloud Run и Cloud Run Job — две оболочки над одним `pipeline.run`. Синхронный путь
отвечает за демонстрируемость (34 секунды на статью), фоновый — за то, что просит
хакатон: работа над корпусом без человека в цикле. Результат каждой статьи пишется в
GCS отдельным объектом, поэтому упавшая статья не уносит прогон.

---

## Путь A и путь B

```mermaid
flowchart LR
    A["Статья открыта<br/>27.5% класса"] --> A1["Europe PMC<br/>JATS + .docx"] --> A2["<b>L1</b><br/>полный разбор"]
    B["Статья закрыта<br/>Cloudflare / Elsevier"] --> B1["только абстракт<br/>из метаданных"] --> B2["<b>L3</b><br/>предварительный разбор"]
    B --> B3["Пользователь приносит файлы<br/>POST /analyze/upload"] --> A2

    style A2 fill:#e6f4ea,stroke:#34a853
    style B2 fill:#fef7e0,stroke:#fbbc04
    style B3 fill:#e6f4ea,stroke:#34a853
```

Покрытие измерено на выборке из 40 статей класса (F-21, F-24, F-25): Europe PMC отдаёт
full-text для 27.5%, и там, где отдаёт, приложения приходят в 9 случаях из 11 — всегда
`.docx`, PDF ни разу. Добавление прямых PDF у издателей, публикующих машиночитаемый
указатель `citation_pdf_url`, доводит автоматическую добычу примерно до 55%. Остальное
— за Cloudflare (13 из 34) и у Elsevier за TDM-токеном; защиту сайтов не обходим.

Отсюда путь B: пользователю статья часто доступна лично, даже когда она недоступна
машине. Реализован эндпоинтом `POST /analyze/upload` (`truth/pdf_tables.py`, pdfplumber).
**Проверено на статье, доступной обоими путями** (`10.3389/fonc.2026.1742210`): путь B
даёт тот же уровень L1, те же 5 основных и 2 приложенных таблицы и ноль непроверенных
чисел — то есть принесённый файл действительно равноценен автоматической добыче.

---

## Чего на схеме нет

Честный список, чтобы диаграмма не обещала больше кода:

- **ADK-обёртки нет.** Требование R2 закрыто через `google-genai` SDK; оркестратор
  свой (`pipeline.py`). Переход на ADK — резерв Р-1 в `TODO.md`.
- **Суб-агентов два, и оба заведены по замеру.** `baseline_comparability` — после того
  как основной критик трижды из трёх не взял пункт про парадокс исходной тяжести (F-45).
  `time_related_biases` — после того как внешний эталон дал 25% против 92% на своём
  (F-47); с ним разрыв между кейсами схлопнулся с 67 п.п. до 4.5 (F-48). Для остальных
  пяти доменов провала не измерено, поэтому агентов там нет.
- **Memory Bank не подключён** — Q-11 открыт.
- **Document AI не подключён и не понадобился.** Q-08 закрыт иначе: pdfplumber (MIT)
  разбирает таблицы текстовых PDF локально и бесплатно. Для сканов без текстового слоя
  вопрос остаётся открытым — такой файл честно отклоняется с пометкой «нужен OCR».
