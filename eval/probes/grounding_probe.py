"""
Зонд `checkGrounding` — измерение готового API Google. **В продукте не используется.**

Модуль лежит в `eval/probes/`, а не в пакете `truth/`, потому что замер 29.08
показал: на полной статье слой не различает верное и выдуманное вовсе. Тот же
набор из 200 фактов документа `10.1136/jitc-2025-014726`, четыре кандидата:

| Кандидат | supportScore |
|---|---|
| обоснование домена «Confounding» (верное) | 0.010 |
| обоснование домена «Measurement of the exposure» (верное) | 0.003 |
| «time zero совмещён с первым назначением ICI» (верное) | 0.003 |
| выдумка: «4 812 случаев миокардита 4 степени, испытание остановлено» | **0.003** |

Выдумка и правда получили один и тот же балл. Пробовались три способа сборки
фактов — строки текста, строки таблиц с заголовками (`row_to_fact`), их
объединение: 0.0039 / 0.0034 / 0.0034. Дело не в качестве фактов.

Отдельно проверялся вывод `overall.summary`: 0.0039 при трёх claim и нуле
процитированных. Это объяснимо — наш вывод есть **критика методологии**, а не
пересказ статьи, и подтверждать его в тексте статьи нечем. Опубликовать такой балл
рядом с верным разбором значило бы ложно обвинить собственную выдачу — ровно то,
за что этот продукт критикует чужие работы.

Замер F-52 (ниже) остаётся в силе: на одной приложенной таблице McDonald API
выдумку ловил. Вывод не «API плохой», а «на нашем входе он не работает»: разница
между 6 фактами одной таблицы и 200 фактами целой статьи.

Ниже — исходная докстрока с замером 28.08.
