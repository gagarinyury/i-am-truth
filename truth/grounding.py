...
# Шаблон числа берётся из верификатора, а не переписывается здесь. Три копии этой
# строки (тут, в `pipeline` и в `cells`) прожили до 31.08 под видом одинаковых, а
# были разными: в классе разделителей у одной стоял неразрывный пробел, у двух
# других — обычный дважды. И ни одна не видела знака, из-за чего «-0.69» распадалось
# на «0.69» и разность рисков с перепутанным направлением считалась подтверждённой.
...
# Статусы, при которых число не может служить опорой вывода: его в документе либо
# нет вовсе, либо оно принадлежит другой группе, либо взято с обратным знаком.
CONTESTED = {"UNVERIFIED", "GROUP_MISMATCH", "SIGN_MISMATCH"}
...
def index(claims: list) -> dict:
    ...
    for c in claims:
        ...
        # `found` здесь означает «подтверждает вывод», а не «встречается в
        # документе». Число, приписанное другой группе или взятое с обратным
        # знаком, в документе есть — и именно поэтому опорой быть не может.
        # Сводка верификатора считает такие найденными: там вопрос другой —
        # есть ли вообще такое значение в статье. Расхождение намеренное.
        found = c.get("status") not in CONTESTED
        prev = out.get(v)
