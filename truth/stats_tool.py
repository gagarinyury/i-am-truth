...
class TwoByTwo:
    ...
    def odds_ratio(self) -> float | None:
        ...
        Условий четыре, а не два, и это тоже правка по замеру. Страж покрывал
        `b` и `c`, но не `d`: при исходе у всех в КОНТРОЛЕ формула честно
        доходила до конца и возвращала `0.0` — предел верный, но выданный как
        обычное значение, мимо списка `undefined`. Зеркальные вырожденные
        таблицы отвечали по-разному: насыщенная рука экспозиции давала «не
        определено», насыщенный контроль — «ноль». Правило «величина, которой
        нет, выражается отсутствием» либо симметрично, либо это не правило.
        """
