...
class AnalyzeRequest(BaseModel):
    ...
    # Медианы судьи по всем сохранённым прогонам репозитория (31.08):
    ...
    # Что измерено твёрдо — время: ADK шёл 137 и 291 секунду там, где прямой путь
    # укладывается в 28.3-128.4 (медиана 40.5 по 36 прогонам). Поэтому выбор явный,
    # а по умолчанию стоит быстрый путь. Подробнее — в докстроке `pipeline.run`.
    engine: str = "direct"
...
def levels():
    ...
    return {
        ...
        # Цена уровня и цена продукта — два разных замера, и раньше отсюда уходил
        # только первый, без указания, чем он получен. Теперь рядом стоят оба.
        "end_to_end": retrieval.END_TO_END,
        "note": "Цены уровней получены замером на подготовленных входах харнесом "
                "(поле measured_by), а цена продукта целиком — в end_to_end. "
...
# Идентификатор попадает в путь файла и в имя объекта GCS. Starlette не пропустит
# слэш в параметре пути, так что дороги наружу сейчас нет — но это чужое свойство,
# а не наша проверка, и держать безопасность на нём значит зависеть от того, чего
# мы не контролируем. Формат `store.new_id` — «audit-ГГГГММДД-ЧЧММСС-хеш»; шаблон
# ниже шире, чтобы старые идентификаторы остались читаемыми.
AUDIT_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$")


def _valid_id(audit_id: str) -> str:
    if not AUDIT_ID.match(audit_id or "") or ".." in audit_id:
        raise HTTPException(400, "недопустимый идентификатор разбора")
    return audit_id


@app.get("/audits/{audit_id}")
def audit(audit_id: str):
    r = store.get(_valid_id(audit_id))
    if r is None:
